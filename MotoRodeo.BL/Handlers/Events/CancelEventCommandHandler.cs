using DMCorp.Framework.Basics.DAL;
using DMCorp.Framework.Basics.Security;
using MediatR;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Logging;
using MotoRodeo.BL.Commands.Events;
using MotoRodeo.DAL.Entities;
using MotoRodeo.DAL.Enums;

namespace MotoRodeo.BL.Handlers.Events;

/// <summary>
/// Отмена планируемого мероприятия (Planned → Completed).
/// </summary>
public sealed class CancelEventCommandHandler(
    IUnitOfWork unitOfWork,
    IAdvancedSecurityService security,
    ILogger<CancelEventCommandHandler> logger) : IRequestHandler<CancelEventCommand>
{
    /// <inheritdoc />
    public async Task Handle(CancelEventCommand request, CancellationToken cancellationToken)
    {
        var (canManage, isJudge) = await EventLifecycleRules.RequireEventManagerOrJudgeAsync(
            unitOfWork, security, request.EventId, cancellationToken);

        logger.LogInformation("Отмена события. EventId={EventId}", request.EventId);

        var entity = await unitOfWork.Query<DBEvent>()
            .SingleOrDefaultAsync(x => x.Id == request.EventId, cancellationToken)
            ?? throw new KeyNotFoundException("Событие не найдено.");

        var now = DateTimeOffset.UtcNow;
        if (!EventLifecycleRules.CanCancel(entity, canManage, isJudge, now))
        {
            throw new InvalidOperationException(
                "Отменить можно только планируемое мероприятие.");
        }

        entity.Status = EventStatusEnum.Completed;
        await unitOfWork.SaveChangesAsync(token: cancellationToken);

        logger.LogInformation("Событие отменено. EventId={EventId}", request.EventId);
    }
}
